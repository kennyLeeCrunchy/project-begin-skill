...
默认在当前分支开发，不主动创建或切换 branch / worktree；用户明确要求独立开发时再开 branch，需要并行修改时再为任务增加独立 worktree。创建 branch 本身不要求先提交，但要把当前未提交改动带到新的 branch / worktree 时，应先提交；stash 仅在用户明确要求时使用。提交前只暂存本任务文件，并运行与改动范围匹配的验证。这样可以在不增加日常操作成本的前提下，保留 Git 历史的可追溯性。
...
`project-begin` 负责建项目或用户明确要求的文档修复；项目文档生成后，日常任务路由和 Git 决策直接遵循 `AGENTS.md` 与 `/docs`，不会每次开发都重新调用这个 skill。
